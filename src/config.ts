import { Config as MangoConfig, IDS as MANGO_MARKETS } from '@blockworks-foundation/mango-client'
...
export function loadMarkets(protocol: Protocol): Record<string, string> {
  switch (protocol) {
    case 'mango': {
      const mangoGroupConfig = new MangoConfig(MANGO_MARKETS).getGroup('mainnet', 'mainnet.1');
      return Object.fromEntries(mangoGroupConfig!.spotMarkets.map((spotMarketConfig) => {
        return [spotMarketConfig.name, spotMarketConfig.publicKey.toBase58()]
      }));
    }
    case 'serum': {
      const programId = loadProgramId(protocol);
      return Object.fromEntries(SERUM_MARKETS.filter((marketConfig) => { return marketConfig.deprecated == false && marketConfig.programId.toBase58() === programId }).map((marketConfig) => {
